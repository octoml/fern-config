...
If you don't already have a container in hand, [see our guide to building a custom endpoint with our CLI.](../create-custom-endpoint-using-the-cli.mdx)
...
* **Registry Credential:** Registry Credential defaults to Public, which indicates that your container is available for anyone on the internet to pull. If your container is instead stored in a private registry, follow the guide in [Pulling containers from a private registry](/bring-your-own-model/create-custom-endpoints-from-a-container/pulling-containers-from-a-private-registry) to store your registry credentials so that we can pull the private container.
* **Health Check Path:** See [Health Check Path in custom containers](../health-check-path-in-custom-containers.mdx)
...
* **Specify Secrets:** Provide secrets to mount onto the container, such as database secrets that you want to reuse across endpoints. Follow the guide in [Setting up account-wide secrets for your custom endpoints](setting-account-wide-secrets-for-custom-endpoints.mdx) to set these up.
...
* **Select hardware:** We offer three tiers of hardware-- see [Hardware, Pricing, and Billing](../../getting-started/pricing-and-billing.mdx) for more information. Your choice of hardware determines the pricing that you will pay for your endpoint.
...
* You need to edit the curl to use your own API token. If you don't have an existing token, refer to [how to create an OctoAI API Token](../../getting-started/how-to-create-an-octoai-access-token.mdx)
