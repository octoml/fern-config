...
Follow [CLI & SDK Installation](/compute-service/cli-and-sdk-installation) to make sure you have the CLI and SDK installed. This guide assumes you are running the latest version of the `octoai` CLI.
...
For more information about secrets and environment variables, see [Setting up secrets or environment variables for your custom endpoints](create-endpoint-from-existing-container/setting-account-wide-secrets-for-custom-endpoints.mdx).
...
3. **Provide your registry credentials to the OctoAI Compute Service**. To provide registry credentials using the web UI, see [Pulling containers from a private registry](create-endpoint-from-existing-container/pulling-containers-from-a-private-registry.mdx). To provide registry credentials using the `octoai` CLI, use the `octoai regcred create` command. To verify your registry credentials were created correctly, use the `octoai regcred list` command.
