...
* Two server routes that OctoAI containers should have:  
   ...
   * a route for health checks (e.g. "`/healthcheck`"). See [Healthcheck path in custom containers](health-check-path-in-custom-containers.mdx) for a detailed explanation.
...
Now that you have your container, [create an endpoint](create-endpoint-from-existing-container/create-custom-endpoints-from-a-container.mdx) to establish your endpoint on OctoAI.