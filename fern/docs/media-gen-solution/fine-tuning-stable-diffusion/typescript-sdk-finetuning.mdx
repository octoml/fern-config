...
Please see [Fine-tuning Stable Diffusion](/docs/api-reference/fine-tuning/create-tune) for more specifics about each parameter in the fine-tuning API, using curl, or the Python SDK. Our [Asset Library in the TypeScript SDK](/docs/api-reference/assets/list-assets) documentation goes more into the specifics of using different asset methods as well, and our [TypeScript SDK Reference](https://octoml.github.io/octoai-typescript-sdk/) covers each parameter and method in more detail.
...
[Asset Library in the TypeScript SDK](/docs/api-reference/assets/list-assets) covers more specifics about the methods, so this example will be focused on a code snippet for uploading multiple files in a folder at once. There are different approaches, such as making the asset name match the name of the file, however for our purpose we are going to keep everything named with our `NAME` constant in order to make it easier to search and delete the assets created later.
