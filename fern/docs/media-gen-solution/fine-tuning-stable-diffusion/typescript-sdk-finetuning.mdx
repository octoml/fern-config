...
Please see [Fine-tuning Stable Diffusion](../../api-reference/fine-tuning/create-tune.mdx) for more specifics about each parameter in the fine-tuning API, using curl, or the Python SDK. Our [Asset Library API reference](../../api-reference/assets/list-assets.mdx) documentation goes more into the specifics of using different asset methods as well.
...
- If you use the `OCTOAI_TOKEN` envvar for your token, you can instantiate the client with `const octoai = new OctoAIClient()` or pass the token as a parameter to the constructor like `const octoai = new OctoAIClient({ apiKey: process.env.OCTOAI_TOKEN })`.
...
The [Asset Library API reference](../../api-reference/assets/list-assets.mdx) covers more specifics about the methods, so this example will be focused on a code snippet for uploading multiple files at once. For this guide, we will be uploading all files located in a local folder.
