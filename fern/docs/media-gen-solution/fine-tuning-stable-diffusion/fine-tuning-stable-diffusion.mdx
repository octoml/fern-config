...
Complete fine-tuning API parameters are organized in our [API Reference documentation](/docs/api-reference/fine-tuning/create-tune).
...
Full API details and parameters [are available here](/docs/api-reference/fine-tuning/create-tune). Using the `continue_on_rejection` boolean parameter, you can optionally continue with the fine-tune job if any of the training images are identified as NSFW.
