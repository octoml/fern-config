...
* I can't load my checkpoint file, and in the Logs tab in the UI I see this error message: `safetensors_rust.SafetensorError: Error while deserializing header: MetadataIncompleteBuffer`. What should I do?  
   * This error occurs if your checkpoint happens to be extra large, in which case instead of using the example model which has been pre-optimized for Medium tier hardware (A10s), you need to run the UI on a Large Tier hardware (A100) instead. Follow [Create your own Endpoint from an existing container](/bring-your-own-model/create-custom-endpoints-from-a-container/create-custom-endpoints-from-a-container) to make your own Automatic1111 endpoint on a Large Tier hardware, using this container image `258175030240.dkr.ecr.us-east-1.amazonaws.com/template-images:automatic1111-v0.01` with port 3000 and no healthcheck.
