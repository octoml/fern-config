# Text Gen Fine-tuning CLI Guide

| title | subtitle | slug |
| --- | --- | --- |
| Text Gen Fine-tuning  | Create custom assets with OctoAI's fine-tuning of Text Gen models. | text-gen-solution/fine-tuning/text-gen-fine-tuning-cli-guide

## **How to Create a Fine-Tuned LoRA Using OctoAI's Command Line Tools**
...
Please refer to the [Text Gen Fine-tuning CLI Reference Guide](/docs/cli/text-gen-fine-tuning-cli-reference-guide) for more details on each parameter in the fine-tuning CLI. Additionally, our [Asset Library API reference](https://octo.ai/docs/api-reference/asset-library/list) provides further specifics on using various asset methods.

Visit our [pricing page](https://octo.ai/pricing/) to learn more about how fine-tuned model training and usage are billed.

OctoAI now supports creating fine-tuned LoRAs for the following models:

- Meta-Llama-3-1-8b-Instruct (8K token context)
- Meta-Llama-3-1-70b-Instruct (8K token context)
...
3. **checkpoint** (required): Specify a checkpoint for a particular model. Presently, we support only Llama-3.1 8B and Llama-3.1 70B with an 8K token context. For Llama 3.1 8B Instruct, use `"octoai:meta-llama-3-1-8b-instruct"`. For Llama 3.1 70B Instruct, use `"octoai:meta-llama-3-1-70b-instruct"`. 
...
6. **weight and bias parameters** (optional): Enable with "--wandb-project-name" and "--wandb-secret-key”. To use this feature, sign up for [Weights and Biases](https://wandb.ai/site) and follow the [Weight & Biases Setup Guide](../fine-tuning/weight-and-biases-setup-guide) .
...
- In the `"model"` parameter, use `"meta-llama-3.1-8b-instruct"`or `"meta-llama-3.1-70b-instruct"`.
- In the `"peft"` field, use the `tune_name` of the fine-tuning job. You can use `octoai tune list` to find a list of tuned jobs.
- For details of other parameters, refer to the [inference guide](https://octo.ai/docs/api-reference/octoai-api/inference).
...
If you want to send a meta-llama-3.1-70B inference request, change the `"model"` parameter to `"meta-llama-3.1-70b-instruct"`.
